...
class CrazyflieController(Node):
    def __init__(self):
        ...
        self.T_land = {}
        self.comp_times = []
        self.arrivals = np.zeros((1, len(self.drone_bodies)))
        ...
        self.takeoff_solver_data = setup_takeoff_solver(solver_config)
        self.solver_data = setup_spline_solver(solver_config)
    ...
    def load_params(self, config_file):
        ...
        self.Tto = 20  # Takeoff time in seconds
    ...
    def load_mpc_configuration(self, config_file):
        ...
        solver_config = {'Na': len(self.drone_bodies), 'Npred': params['Npred'], 'Npred_to': params['Npred_to'], 'Ts': self.Ts,
                         'Q': np.array(params['Q'], dtype=np.float32), 'R': np.array(params['R'], dtype=np.float32),
                         'Vc': Vc, 'rmin': params['rmin'], 'spline_deg': params['spline_deg'], 'n_ctrl_pts': params['n_ctrl_pts'],
                         'Qto': np.array(params['Qto'], dtype=np.float32), 'Rto': np.array(params['Rto'], dtype=np.float32),
    ...
    def check_arrival(self, pos_ref, state):
        return int(np.linalg.norm(pos_ref - state) < 0.09)
    ...
    def control_callback(self):
        ...
        if self.flight_phase == "takeoff":
            tic = time.time()
            cf_control, landing_args = self.control_loop(self.current_step, self.refto, self.vrefto)
            toc = time.time()
            ...
            # if all(self.arrivals) == True and self.current_step >= 100:
            if self.current_step >= 100:
                self.flight_phase = "flight"
                self.current_step = 0  # Reset step for normal trajectory
            elif all(self.arrivals) == False and self.current_step == len(self.refto[self.uris[0]]):
                self.flight_phase = "landing"
                self.current_step = 0
                # The drones have not successfully arrived to the starting point of the trajectory at the end of the takeoff time, therefore land and stop the experiment
            ...
            if done == 1:
                cfcontrol.save_data(f"src/Data_Drone/data_files/drone_data_{self.simid}_spline.npy", self.output_data, self.ref, self.vref)
                cfcontrol.save_data(f"src/Data_Drone/data_files/drone_data_{self.simid}_takeoff_spline.npy", self.takeoff_data, self.refto, self.vrefto)
                np.save(f"src/Data_Drone/data_files/computation_times_{self.simid}_spline.npy", {"comp_times": self.comp_times})
    ...
    def control_loop(self, cnt, ref_set, vref_set):
        ...
        for i, drone in enumerate(self.drone_bodies):
            ...
            if self.flight_phase == "takeoff":
                if self.check_arrival(ref_set[self.uris[i]][cnt, 0:3].reshape(1, -1), state_xi[0:3].reshape(1, -1)) == 1:
                    self.arrivals[i] = 1
                v = compute_spline_control(self.takeoff_solver_data, state_xi.T, ref_set[self.uris[i]], vref_set[self.uris[i]], cnt)
            else:
                v = compute_spline_control(self.solver_data, state_xi.T, ref_set[self.uris[i]], vref_set[self.uris[i]], cnt)
